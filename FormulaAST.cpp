    ...
    namespace {
        ...
        public:
            ...
            double Evaluate(std::function<double(Position)> args) const override {
                ...
                case Divide:
                    double denominator = rhs_->Evaluate(args);
                    if (denominator == 0 || !std::isfinite(denominator)) {
                        throw FormulaError(FormulaError::Category::Div0);
                    }
                     result = lhs_->Evaluate(args) / denominator; 
                     break;
                }
        ...
        public:
            ...
            double Evaluate(std::function<double(Position)> lambda) const override {
                return lambda(*cell_);
            }
        ...
        public:
            ...
            {
                std::string str = ctx->CELL()->getSymbol()->getText();
                Position value = Position::FromString(str);
...
double FormulaAST::Execute(std::function<double(Position)> lambda) const {
    return root_expr_->Evaluate(lambda);
}
