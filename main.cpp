...
namespace {
    ...
    void TestSetCellPlainText() {//bad optional access
        auto sheet = CreateSheet();
    ...
    void TestFormulaReferences() {//bad optional access
        auto sheet = CreateSheet();
        auto evaluate = [&](std::string expr) {
            return std::get<double>(ParseFormula(std::move(expr))->Evaluate(*sheet));
            };
    ...
    void TestErrorValue() {//bad optional access
        auto sheet = CreateSheet();
    ...
    void TestFormulaInvalidPosition() {//invalid position
        auto sheet = CreateSheet();
